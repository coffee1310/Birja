...
createChart();
        ...
        try {
            ...
            if (data.Response === "Success") {
                ...
                for (let i = data.Data.Data.length; i > 0; i -= interval) {
                    const intervalData = data.Data.Data.slice(i, i + interval); // Здесь
        ...
        try {
            ...
            if (loadedData.length > 0) {
                ...
                if (timeDifference >= intervalMilliseconds) {
                    ...
                    if (Math.log10(volumePoint) < val) {
                        while (Math.log10(volumePoint) < val) {
                            if (volumePoint === 0) {
                                volumePoint += newDataPoint.c;
                                break;
                            }
                            volumePoint *= 10;
                    ...
                    if (currentChart.data.labels) {
                        ...
                        if (line_annotation) {
                            line_annotation.xMin = currentChart.data.datasets[0].data.at(-1).x;
                        }
                        volumeChart.update();
                        currentChart.update();
                    }                   
            ...
            if (volumeChecked) {
                ...
                const intervalInMilliseconds = convertIntervalToMilliseconds(interval, timeUnit);
                if (chartType === 'line') {
    ...
    function updateChart(chartType, timeUnit) {
        ...
        if (chartType === 'line') {
            ...
            volumeChart.data.datasets[0].data = visibleData.map(point => ({
                x: new Date(point.time * 1000),
                y: point.volume,
            }));;
            volumeChart.update();
    ...
    document.getElementById('cryptoChart').addEventListener('wheel', async (event) => {
        ...
        line_annotation.xMin = currentChart.data.datasets[0].data.at(-1).x;
        label_annotation.xValue = currentChart.options.scales.x.max;
    ...
    fetchAndAddDataToChart(cryptoSelect.value, currentRange, chartType);
    updateAnnotationsWithNewBounds(currentChart);
    updateChart('candlestick', 'hour');
    setInterval(() => fetchAndAppendDataToChart(cryptoSelect.value, currentRange, chartType), 1000);
    ...
    function addHorizontalLine(type) {
        ...
        const lineAnnotation = {
            ...
            xMin: currentChart.data.datasets[0].data.at(-1).x,
            borderColor: color,
            borderWidth: 2,
        };
    ...
    function startTimer() {
        ...
        const timer = setInterval(() => {
            ...
            let currentPrice;
            if (chartType === "line") {
                currentPrice = currentChart.data.datasets[0].data.at(-1);
            } else {
                currentPrice = currentChart.data.datasets[0].data.at(-1).c; // Текущая цена из последних данных
            }
